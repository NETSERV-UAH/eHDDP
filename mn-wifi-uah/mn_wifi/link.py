...
class IntfWireless(Intf):
    ...
    def setTxPower_intermidiate(self, txpower):
        self.iwdev_cmd('{} set txpower fixed {}'.format(self.name, txpower * 100))
        debug('\n')
        self.txpower = txpower
        self.setDefaultRange()
...
class WirelessLink(TCIntf, IntfWireless):
    ...
    def config(self, bw=None, delay=None, jitter=None, loss=None,
        ...
        info("at map stage w/cmds: %s\n" % cmds)
        tcoutputs = [self.tc(cmd) for cmd in cmds]
        for output in tcoutputs:
            if output != '':
                error("*** Error: %s" % output)
        info("cmds:", cmds, '\n')
        info("outputs:", tcoutputs, '\n')
...
class adhocmultinodes(LinkAttrs):

    node = None

    def __init__(self, node, nodes=[], intf=None, intfs=[], ssid='adhocNet',
                 channel=1, mode='g', passwd=None, ht_cap='',
                 proto=None, ibss="02:CA:FF:EE:BA:01", node_src = True, 
                 proto_args='', loss = 0.00, **params):
        ...
        self.channel = channel
        self.ht_cap = ht_cap
        self.associatedTo = 'adhocmultinodes'
        self.ibss = ibss
        self.link = intf.link = self
        self.proto = proto
        ...
        self.node.mount_bfs()
        #debug("cd ~/eHDDP/filter_packet_xdp && ./xdp_loader -d", intf.name," -F --progsec xdp_filter_packet --auto-mode\n")
        self.node.cmd("cd ~/eHDDP/filter_packet_xdp && ./xdp_loader -d", intf.name," -F --progsec xdp_filter_packet --auto-mode && cd")      

        if (node_src):
            position = 0    
            for nxt_node in self.nodes:
                nxt_nodes = list(nodes)
                nxt_intfs = list(intfs)
                if nxt_node.name != self.node:
                    nxt_nodes[position] = self.node
                    nxt_intfs[position] = self.intf
                    adhocmultinodes(nxt_node, nodes=nodes, intf=intfs[position], 
                        intfs=intfs, ssid=ssid,
                        channel=channel, mode=mode, passwd=passwd, ht_cap=ht_cap,
    ...
    def configurecommunication(self):
        for node_dst in self.nodes:
            if node_dst.name != self.node:
                wlan = node_dst.params['wlan'].index(self.intfs[self.nodes.index(node_dst)])
                intf = node_dst.wintfs[wlan]
