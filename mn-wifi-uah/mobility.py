...
from mn_wifi.link import ConfigWLink, mesh, adhoc, ITSLink, master, adhocmultinodes
        ...
        for node in nodes:
            for intf in node.wintfs.values():
                if isinstance(intf, adhoc) or isinstance(intf, mesh) or isinstance(intf, ITSLink):
                    pass
                #MODIFICACION UAH
                elif isinstance(intf,adhocmultinodes):
                    cmd = 'cd /home/arppath/custom/Pruebas-HDDP/filter_packet_xdp && ./prog_user --dev '+ intf.name
                    allow = ' -L '
                    deny = ' -R '
                    num_accept = 0;
                    num_deny = 0;
